...
gbm_coad_vaesub_mut <- ras_encoded_sub_data_list[[1]] %>%
  dplyr::filter(comparison == 'COAD_GBM') %>% dplyr::select(distance)
gbm_coad_vaesub_wtmut <- wt_mut_enc_sub_data_list[[1]] %>%
  dplyr::filter(comparison == 'COAD_GBM') %>% dplyr::select(distance)

gbm_coad_rawsub_mut <- ras_raw_sub_data_list[[1]] %>%
  dplyr::filter(comparison == 'COAD_GBM') %>% dplyr::select(distance)
gbm_coad_rawsub_wtmut <- wt_mut_raw_sub_data_list[[1]] %>%
  dplyr::filter(comparison == 'COAD_GBM') %>% dplyr::select(distance)

t.test(gbm_coad_vaesub_mut, gbm_coad_vaesub_wtmut)
t.test(gbm_coad_rawsub_mut, gbm_coad_rawsub_wtmut)
